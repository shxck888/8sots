...
**Build 1 的登入、Employee Master、排班、GPS 原始打卡、Attendance 與假勤申請第一版已完成 production 驗證**。Next.js 應用、Supabase production schema、Vercel deployment 與 `hrms.8sots.com.tw` 已建立。打卡採伺服器正式時間；Attendance 以版本化快照重算，原始 Punch 永不覆寫。
...
- `lib/database.types.ts` 已同步 production 已驗證的 `016` schema，server session 與 Auth Admin clients 均使用 application `Database` generic；migration-to-types drift contract 已建立。
...
- Vercel production code commit `9982787` 已 Ready；Deployment Resources 確認各動態 Function 位於 `HND1`，正式網域 `/`、`/my-schedule`、`/attendance` 與管理頁保有登入 session 且回傳 `200`。動態 route loading、aggregate read RPC、request-scoped cache、Tokyo compute locality 與手機可讀性均有 automated contract。
...
- 管理員 Attendance Day drill-down 已完成：可查看每個班段的排班／有效打卡／實際分鐘、異常、原始 GPS evidence 與當日補卡申請；核准補卡晚於最新快照時持續顯示「需要重新計算」並預填受影響日期。
- 申請中心第一版已上線：已連結員工可在 `/requests` 提出請假或加班並查看狀態；`request.manage` 管理員可在 `/admin/requests` 核准或拒絕。Request 與單次 final Decision 分表、只經 audited RPC mutation；假別目前僅分類，不代表額度、扣薪或薪資認列。
...
| Quality | ESLint、TypeScript strict、Vitest（100 tests）、Next production build；Database types drift contract；navigation performance contract；Supabase production rollback-only RLS／Schedule／Punch／Attendance／Request integration scripts |
...
- `app/requests/`、`app/admin/requests/`、`lib/work-request-contract.ts`、`202608250016_request_center.sql`：請假／加班申請、審核、RLS 與稽核邊界

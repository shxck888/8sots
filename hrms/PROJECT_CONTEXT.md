...
**Build 1（Auth, tenant and employee foundation）已完成，排班 domain foundation 已完成**。Next.js 應用、Supabase production schema、Vercel deployment 與 `hrms.8sots.com.tw` 已建立。自訂帳號登入、管理員後台、完整 Employee Master、員工帳號生命週期、跨租戶隔離，以及班別／班表版本安全邊界皆已通過 production 驗證。
...
- Supabase production 已套用並在 migration history 確認 `202608240001` 至 `202608250010`；`010` 建立 tenant-wide Shift/Shift Segment、Schedule Version/Assignment、`schedule.manage`、RLS、audited RPC 與發布後不可變保護。
...
- 目前程式通過 ESLint、TypeScript、61 項 Vitest 與 Next.js production build。

### IN PROGRESS

- 排班管理 UI 尚未建立；首頁仍以代表性假資料呈現班表、出勤與打卡，按鈕尚未連接 domain operation。

### PLANNED

- 排班管理日曆、草稿編輯／發布 UI，以及 Password recovery、邀請、MFA、打卡、出勤與後續業務模組。
...
| Quality | ESLint、TypeScript strict、Vitest（61 tests）、Next production build；Database types drift contract；Supabase production rollback-only RLS／Schedule integration tests |
...
- **PLANNED:** Schedule 管理 UI、Password Recovery/MFA、Attendance、Leave、Overtime、Approval、Payroll、Insurance、Notification、Report。
...
登入、員工主檔、員工帳號生命週期、跨租戶隔離、typed data-access 與排班 database foundation 已完成 production 驗證。下一階段建立無門市前提的管理後台排班日曆、草稿指派與發布流程。

...
import { KeyRound, PencilLine, PauseCircle, PlayCircle, UserRoundPlus } from "lucide-react";
...
export function EmployeeAccountPanel({
  ...
  const [usernameState, usernameAction, changingUsername] = useActionState(
    changeEmployeeUsername.bind(null, employeeId), initialState,
  );
    ...
    <section className="admin-panel account-panel">
      ...
      {!account ? (
        <form action={provisionAction} className="account-form">
          <p>為此員工建立獨立登入帳號。建立後管理員仍可變更帳號；密碼不會被保存或顯示。</p>
        ...
        <div className="account-management">
          ...
          <div className="account-credential-actions">
            <form action={usernameAction} className="account-inline-form">
              <Feedback state={usernameState} />
              <div className="admin-field"><label htmlFor="changeUsername">新登入帳號</label><input id="changeUsername" name="username" defaultValue={account.username} autoComplete="off" minLength={3} maxLength={32} pattern="[A-Za-z0-9_]+" required />{usernameState.fieldErrors?.username?.[0] ? <p className="admin-field-error">{usernameState.fieldErrors.username[0]}</p> : null}</div>
              <button className="admin-button secondary" disabled={changingUsername} type="submit"><PencilLine size={17} />{changingUsername ? "變更中…" : "變更帳號"}</button>
            </form>

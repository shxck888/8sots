...
export default async function AttendanceDayDetailPage({ params }: {
  ...
  return (
    <>
      ...
      <section className="admin-panel attendance-detail-panel"><header><div><span className="admin-eyebrow">RAW EVIDENCE</span><h2>當日原始打卡</h2></div></header><div className="attendance-evidence-list">{punchesResult.data?.map((record) => <article key={record.id}><span className={`attendance-event ${record.event_type}`}>{(punchActionSchema.safeParse(record.punch_action).success ? punchActionLabels[record.punch_action as keyof typeof punchActionLabels] : punchEventLabels[record.event_type])}</span><div><strong>{formatTaipeiDateTime(record.occurred_at)}</strong><small>{punchSourceLabels[record.source]}</small></div><div><span><MapPin size={14} /> {record.source === "qr" ? "授權機器驗證" : locationVerificationLabels[record.location_verification]}</span>{record.source === "web_gps" && record.accuracy_m != null ? <small>GPS 誤差約 {Number(record.accuracy_m).toFixed(0)} 公尺</small> : null}</div></article>)}{!punchesResult.data?.length ? <div className="admin-empty"><strong>當日沒有原始打卡</strong></div> : null}</div></section>

      {requestsResult.data?.length ? <section className="admin-panel attendance-detail-panel"><header><div><span className="admin-eyebrow">CORRECTIONS</span><h2>補卡申請</h2></div></header><div className="attendance-exception-list">{requestsResult.data.map((request) => <article key={request.id}><Clock3 size={18} /><div><strong>{(punchActionSchema.safeParse(request.punch_action).success ? punchActionLabels[request.punch_action as keyof typeof punchActionLabels] : punchEventLabels[request.proposed_event_type])} · {formatTaipeiDateTime(request.proposed_occurred_at)}</strong><p>{request.reason}</p></div></article>)}</div></section> : null}
    </>
  );
}

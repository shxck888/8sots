  ...
  return <>
    ...
    {selected ? <section className="admin-panel payroll-period"><header><div><span className="admin-eyebrow">SELECTED PERIOD</span><h2>{selected.period_month.slice(0, 7)} · {payrollStatusLabels[selected.status]}</h2><p>結算 {selected.period_start} 至 {selected.period_end} · 發薪 {selected.pay_date ?? "待設定"}</p>{selected.review_note ? <small>核對紀錄：{selected.review_note}</small> : null}</div><div className="payroll-actions">
      ...
      {!entries.length ? <div className="admin-empty"><strong>尚未試算</strong><p>先確認每位員工薪資版本，再按「開始試算」。</p></div> : <div className="payroll-entry-list">{entries.map((entry) => {
        const employee = employeeById.get(entry.employee_id); const items = itemsByEntry.get(entry.id) ?? []; const snapshot = objectValue(entry.source_snapshot); const attendance = objectValue(snapshot.attendance); const compensation = objectValue(snapshot.compensation); const attendanceDays = Array.isArray(attendance.days) ? attendance.days : [];
        return <article key={entry.id}><div><strong>{employee?.employee_no ?? String(snapshot.employee_no ?? "—")} · {employee?.full_name ?? String(snapshot.employee_name ?? "未知員工")}</strong><span>應發 {formatMoney(entry.gross_cents)} · 扣款 {formatMoney(entry.deduction_cents)} · <b>實發 {formatMoney(entry.net_cents)}</b></span><small>出勤 {String(attendance.actual_minutes ?? 0)} 分鐘 · 加班 {String(attendance.approved_overtime_minutes ?? 0)} 分鐘 · 例外 {String(attendance.exception_count ?? 0)} 項</small>{compensation.pay_basis === "hourly" && attendanceDays.length === 0 ? <small className="payroll-warning">時薪制本期沒有已計算出勤日；完成核對前請確認確實未排班，或先補正並重算出勤。</small> : null}{snapshot.partial_period === true ? <small className="payroll-warning">本期含到職、離職或薪資異動，需人工確認比例。</small> : null}{!entry.compensation_version_id ? <small className="payroll-warning">缺少有效薪資設定，無法完成核對。</small> : null}</div><div className="payroll-item-stack">

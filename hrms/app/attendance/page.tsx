...
export default async function AttendancePage() {
  ...
  return (
    <WorkspaceShell activePath="/attendance" canManage={workspace.canManage} displayName={workspace.displayName} email={workspace.email} tenantName={workspace.tenantName}>
      ...
      {(requestsResult.data ?? []).length ? <section className="attendance-summary-list"><header><div><span className="eyebrow">CORRECTIONS</span><h2>我的更正申請</h2></div></header>{(requestsResult.data ?? []).map((request) => { const decision = decisions.get(request.id); return <article key={request.id}><strong>{request.work_date}</strong><span>{punchEventLabels[request.proposed_event_type]} · {formatTaipeiDateTime(request.proposed_occurred_at)}</span><span className={`correction-status ${decision?.decision ?? "pending"}`}>{decision?.decision === "approved" ? "已核准" : decision?.decision === "rejected" ? "已拒絕" : "待審核"}</span><em title={request.reason}>{request.reason}</em></article>; })}</section> : null}
      {!result.employeeId ? (

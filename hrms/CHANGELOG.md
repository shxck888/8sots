...
- 既有排班草稿若週一尚無安排，補上「店休」預設；從已發布班表建立新版草稿時，缺少週一安排也會預設店休，已指定班次或休假不受影響。Migration `046` 已套用 production。
- 新建週班表的週一預設記錄為「店休」；管理員可改選平日班、未排班或休假，員工月曆分別顯示店休與個人休假。Migration `045` 支援店休狀態與週一臨時開店。
- 排班管理新增明確的「休假」選項，與「未排班」分開儲存；員工月曆與週檢視顯示休假，休假日不計入應出勤或時薪薪資所需的出勤日，已發布休假日也不可再提出請假申請。Migration `044` 已套用 production。
- 登入頁新增加入主畫面提示；支援安裝事件的瀏覽器可直接安裝，iOS 與其他瀏覽器顯示對應操作說明，已安裝時不顯示。補齊 192px PWA 圖示。
- 請假申請選擇日期後預設「整日」；可切換「指定時間」申請當日部分時段。整日以所選日期的完整區間送審，額度依生效的標準工時扣抵，跨午夜班段也算在整日內。
- 員工首頁 LED 時鐘亮燈由金黃調整為白色，維持未亮筆畫隱藏。
- 員工首頁 LED 時鐘改用高對比暖金色亮燈，未亮的筆畫不再顯示灰色輪廓。
- 員工首頁即時時鐘改用七段式 LED 數字顯示，保留秒數、伺服器校時與手機滿寬置中排版。
- 員工首頁時鐘改為滿寬置中、顯示秒數並持續更新；定期以伺服器時間校正，無法連線時明確標示裝置時間。
- 員工班表月曆改為週日第一欄，跨月日期與週末標題同步調整。
- 員工「我的班表」新增整月月曆與週檢視切換；月曆可切換月份、點選日期查看完整班段及午休資訊，手機版以七欄緊湊顯示已發布班次。
- 員工首頁移除僅供開發參考的「功能進度」卡片，今日班表在桌面版改為橫跨整列顯示。
...
## 2026-09-22（已於 2026-09-23 套用 production）

- 新增 Migration `041` 修正跨模組資料一致性：跨午夜下班卡延續原工作日，出勤日依各自日期選用並保存生效規則版本。
- 補卡只填補缺少的同類事件；完整班段之外的補卡保留為 evidence 並標記未配對異常，不再靜默取代其他班段。
- 特殊營業日有已發布個人班表時可提出請假，解決週末補班與國定假日排班無法請假的衝突。
- 時薪員工若排班或打卡日期尚未納入薪資來源的出勤快照，禁止核對及鎖定薪資。
- 新增 PGlite 整合測試覆蓋上述四項流程；Migration `041` 後於 2026-09-23 套用 production。

## 2026-09-21

- 新增員工封存／恢復流程：封存會改為離職、停用登入與 tenant membership，並從一般員工及排班名單隱藏；所有歷史紀錄保留。
- 員工管理新增「現有員工／已封存／全部」檢視與封存數量；封存員工只能先恢復後再修改資料或帳號。
- 僅允許永久刪除完全沒有排班、打卡、出勤、申請、薪資、投保、特休或主管關聯的誤建員工，且需再次輸入員工編號確認；自我刪除與具管理角色帳號均會阻擋。
- Migration `036` 已套用 production，封存、恢復與永久刪除皆寫入稽核紀錄。
- 新增週年制法定特休台帳：依到職日於滿 6 個月及每個週年自動取得法定天數，保留取得批次、使用、調整與到期結清狀態。
- 管理員可在 `/admin/settings` 設定特休生效日與一天換算分鐘；法定級距版本化保存，特休給薪比例固定 100%，不再以人工年度額度覆寫。
- 員工 `/requests` 顯示特休可用、已核准、待審與批次到期日；管理員 `/admin/requests` 可查看全員餘額並同步，餘額不足時資料庫會拒絕核准。
- Migration `035` 已套用 production，24 個測試檔／163 項 Vitest、TypeScript、ESLint、Next.js production build 與正式站桌面／手機公開 E2E 全數通過。
- 全站提升桌面與手機字級：一般段落 15px、表單輸入 16px、輔助小字 14px、表格內容 15px，涵蓋登入、員工工作台、出勤、申請、排班、薪資與管理後台。
- 新增管理後台 `/admin/account`「帳號安全」頁，所有具管理權限的帳號皆可在重新驗證目前密碼後變更自己的密碼。
- 密碼變更成功後撤銷所有登入工作階段並導回登入頁；密碼內容不寫入業務資料庫或稽核紀錄。
- 新增 Migration `032` 與 `auth.password_changed` 稽核事件；正式資料庫已套用。
- 21 個測試檔／153 項 Vitest、TypeScript、ESLint 與 Next.js production build 全數通過。

## 2026-08-27

- 已核准補卡現在會取代超出排班班段數的同類原始打卡，不再被異常連點排除於計算之外。
- 實際工時限制在各排班班段內，避免班外打卡將非排班時數誤計為工時。
- 新增 30 秒打卡冷卻時間，保留相同冪等鍵的安全重試並阻止快速連點產生交替紀錄。
- 依海之星政策限制單筆加班申請最多 8 小時；跨日可申請，但以前後端與資料庫共同檢查總時數。
- 依海之星排休政策限制請假僅能涵蓋週二至週五，並禁止國定／公司假日；週一固定公休且週末不可排休。
- 請假表單改為單一日期搭配當日開始／結束時間；多日請假必須分筆申請，資料庫同時禁止跨日請假。
- 依行政院人事行政總處官方資料匯入海之星 2026 年 22 筆具名稱的國定假日／補假，保留來源與匯入稽核紀錄。
- 新增請假／加班時段重疊防呆；待審與已核准申請會保留時段，已撤回或已拒絕則可重新申請，並以員工層級交易鎖避免併發重複送單。
- 需要證明的假別（目前病假）可先送件及後補附件，但沒有附件前不可核准；管理端會顯示「待補證明」，仍可拒絕申請。Migration `027` 已套用 production。
- 第二階段薪資草稿基礎上線：有效日員工月薪、月結週期、出勤來源快照、草稿重算、人工加減項、核對與鎖定、員工端已鎖定薪資單及列印／存成 PDF。未設定的扣薪、保險及稅務規則不自動套用；Migration `028` 已套用 production。
...
- 修正到職日改正後舊特休批次仍計入餘額的問題；保留歷史批次供稽核，但僅計入與目前到職日相符的批次。
- 到職日再修正時可建立同年資的新批次；若舊批次已有核准請假或人工調整，先阻擋修改並提示核對特休台帳。
- 管理員可在原始打卡紀錄多選刪除，需填寫原因並再次確認；權限僅授予平台管理員。
- 刪除採可稽核的作廢方式，原始證據和舊出勤快照保留，作廢打卡從員工紀錄與後續出勤計算排除。
- 出勤管理顯示受作廢影響而需要重算的日期，並加入權限、批次刪除、快照與重算的資料庫整合測試。
